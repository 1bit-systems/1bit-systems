...
```
lemond               :13305 Lemonade                 canonical multimodal / OmniRouter
flm                  :52625 FastFlowLM               XDNA NPU serving lane
1bit-proxy           :13306 1bit proxy               union OpenAI-compatible endpoint
open-webui           :3000  Open WebUI               secondary compatibility UI
gaia                 dynamic GAIA                    primary control surface
```
...
Lists install paths. First visible: CachyOS / Arch through repository `install.sh`.
...
4. Verify (health checks, `1bit status`)
...
| `lemonade-sdk` | Lemonade SDK | Apps point at Lemonade direct `:13305` or the union proxy `:13306`. |
...
| Artifact | Link | Sig |
|---|---|---|
| Install script (CachyOS / Arch) | `install.sh` | SHA256 + GPG `[DECIDE]` |
...
- Images (SDXL)
- Video (Wan 2.2)
- Packaging (`install.sh`, CachyOS / Arch)
...
19. `1bit-proxy HTTP surface (:13306/v1 + :13306/api/v1)`
20. `FastFlowLM and Lemonade routing`
...
42. `Gateway dispatch (1bit-proxy, lemond, FastFlowLM)`
...
51. `install.sh and service wiring`
...
| FastFlowLM | XDNA NPU runtime used by the live serving lane |
...
| Rule A | Core serving stays Python-free; caller/UI carve-outs are isolated |
