...
- [x] **Task 2**: NPU fusion hang -- FIXED, see task2-hang-fixed.md. Root cause: I8Ctx::go() blocked forever on r.wait() (XRT default timeout=0), worker loop was also a stub reporting fake success with zeroed data. Added bounded wait (8s, XRT ERT_CMD_STATE_TIMEOUT) + honest success/failure reporting -- existing npu_broken CPU-fallback machinery now actually engages. Verified hang (exit 124) -> exit 0. Known follow-up: abort() throws an XRT exception that needs catching; why EXEC_CMD never signals is still unexplained.
