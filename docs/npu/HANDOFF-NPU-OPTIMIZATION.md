...
## ROCm GPU Path — Bonsai-1.7B on Radeon 8060S (gfx1151)

### Current Status (2026-07-01): 113 tok/s decode
...
- Plus LM head embedding (token_embd) in TQ2 format on GPU
...
| `model.gguf` (sidecar) | Layer RMS norms + output_norm + token_embd (TQ2, dtype=42) |
...
| Prefill (1 tok) | 10.2 ms (98 tok/s) |
...
- Impact: ~539 MB less weight data per token → ~2 ms faster
- Added `embedding_packed_dev` field + `rcpp_fp16_to_fp32` conversion kernel

**2. Eliminated dead quantize kernel launches (112 per decode)**
- Bonsai TQ2 path reads FP16 activations directly — no `rcpp_quantize_fp16_to_i8` needed
- Removed 84× `hipMemcpy(x_scale)` sync points per token
- Less L1/L2 cache pollution from unnecessary kernels

### Bottleneck Analysis
- Decode is 100% memory-bandwidth-bound
- 436 MB weight read / 50 GB/s = 8.7 ms minimum (matches empirical 8.8 ms)
- Compute would take 0.05 ms for M=1 — negligibile
...
cmake -DCMAKE_BUILD_TYPE=Release -DCMAKE_C_COMPILER=amdclang -DCMAKE_CXX_COMPILER=amdclang++ -DUSE_ROCM=ON -DUSE_CK_GEMM=OFF -G Ninja ..
...
HSA_OVERRIDE_GFX_VERSION=11.5.1 HSA_ENABLE_SDMA=0 \
  ...
  ./bitnet_decode ~/models/bonsai-1.7b-bonsai/bonsai-1.7b.h1b 151645 50
```

### Test Results
| Test | Status |
|------|--------|
| `test_prim_and_attn` | 5/6 pass (RoPE precision benign) |
| `test_ternary_gemm_smallm` | PASS |
| `test_sherry_gemv` | PASS |
| `test_bonsai_gemv` | PASS |
| `test_bonsai_e2e` | PASS (8.8 ms/tok) |

### Key Files
| File | Purpose |
|------|---------|
| `include/rocm_cpp/ck_gemm.h` | Kernel API declarations |
| `include/rocm_cpp/bitnet_model.h` | Model struct with `embedding_packed_dev` |
| `src/prim_kernels.hip` | Core GPU kernels (rmsnorm, RoPE, quant, argmax, fp16→fp32) |
| `src/bonsai_tq2_gemv.hip` | TQ2 ternary GEMV (FP16 in/out) |
| `src/h1b_loader.cpp` | Model loader (h1b + sidecar GGUF) |
| `tools/bitnet_decode.cpp` | Main decoder (~1400 lines) |
| `tools/gguf_to_h1b.cpp` | Converter: F16 GGUF → h1b + sidecar |
| `CLAUDE.md` | Developer knowledge base |
| `docs/gpu/HANDOFF-ROCM-GPU.md` | Detailed GPU handoff |

### Next Steps (GPU path)
1. **Kernel fusion**: Fuse rmsnorm+quantize+GEMV to eliminate ~3-4 ms non-GEMV overhead
2. **Async K/V copy**: Overlap `hipMemcpy` with next layer compute
3. **Text prompt inference**: Acquire `.htok` tokenizer file for `--text` mode
4. **Server mode**: `--server 8080` OpenAI-compatible API
5. **Batch decode**: Speculative decoding or batched beam search for throughput
6. **Port to dGPU with HBM**: Bonsai-1.7B on RX 7900 XTX would be compute-bound at ~2 ms/tok
