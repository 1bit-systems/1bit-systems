...
- [ ] **Task 1**: Router consolidation — extend `token-router`'s `cascade` strategy with rank-by-measured-throughput + fallback behavior; fold `spec_decode` (`1bit-systems/tools/token_router.cpp`) and `content` (`1bit-systems/unified-router.py`) into it as strategies, not separate binaries. Update the router diagram in both READMEs once done.
