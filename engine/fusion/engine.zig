//! Unified FusedEngine — wraps NPU (XRT) and GPU (Vulkan/Metal/CUDA) backends
...
// Named imports from build.zig:
//   "sched"      -> scheduler.zig (re-exports KvPagePool, Scheduler, Request, etc.)
//   "npu_engine" -> npu_engine.zig (re-exports NpuEngine)
const sched_mod = @import("sched");
const npu_engine = @import("npu_engine");

// Fusion submodules (in package path)
...
/// Owns the NPU device (XRT), the GPU device (Vulkan), and the shared KV cache.
pub const FusedEngine = struct {
    ...
    // GPU backend (reserved for future)
    ...
    pub fn init(
        ...
        // Initialize the shared KV cache
        ...
        const cross_memory = try CrossBackendMemory.init(allocator);
        ...
        // Try GPU backend (reserved for future GPU offload)
        engine.gpu_available = false;

        log.info("FusedEngine ready: NPU={} GPU={} policy={s}", .{
            engine.npu_available, engine.gpu_available, @tagName(policy),
        });
