/** NPU Engine v11 — M=32 batch decode. Target: beat FLM on Strix Halo.
 *  FLM Kraken Point: 66.5 tok/s (15ms/tok).
 *  Target: >100 tok/s (<10ms/tok) effective at M=32. */
#include <cstdio>
#include <cstdlib>
#include <cstring>
#include <cmath>
...
// OpenMP-parallel LM head: compute logits + sample + top-K in one pass over embedding table
inline void lm_topk_omp(const float*hidden,float*lg,int*top_ids,int K){
    float mx=-1e30f;
    #pragma omp parallel for reduction(max:mx)
    for(int n=0;n<NV;n++){double s=0;const float*__restrict e=&emb_f32[(size_t)n*H];const float*__restrict h=hidden;
    ...
    struct TI{int id;float v;};TI*top=(TI*)alloca(K*sizeof(TI));
...
int main(int argc,char**argv){
    ...
    printf("=== NPU Engine v11 — M=%d Batch Decode ===\n",BS);
    printf("Target: beat FLM (66.5 tok/s Kraken Point → ~130 tok/s Strix Halo)\n\n");
    ...
    // Boot: single-token decode → top-32
    {
        ...
        printf("  [0] boot=%d top32 first 4: %d,%d,%d,%d (%.0fms)\n",top_ids[0],top_ids[0],top_ids[1],top_ids[2],top_ids[3],t_boot);
    }
    ...
    while(step<ng){
        ...
        printf("  [%d] batch=%d tok=%d %.0fms (lm=%.0fms, %.0f ms/tok)\n",step,batch_size,top_ids[0],batch_ms,lm_ms,batch_ms/batch_size);
        step+=batch_size;
    }
    ...
    printf("FLM Kraken Point: 66.5 tok/s (15.0 ms/tok)\n");
