/** NPU Engine v10 — LM head on NPU. N=30720 xclbin, 5 chunks × 1334μs = 6.7ms.
 *  Targets: <10ms/tok batch step, <15ms/tok effective. */
#include <cstdio>
#include <cstdlib>
#include <cstring>
#include <cmath>
...
static std::vector<float> lm_head_f32; // untied lm_head.weight (NV x H), separate from input embedding table
...
// NPU LM head: 5 chunks × N=30720 = covers 153600 ≥ NV=151936
// Embedding table pre-packed at startup. B BOs pre-synced. Only A and C synced per token.
struct LMCtx{
    std::unique_ptr<xrt::xclbin> xc; std::unique_ptr<xrt::hw_context> hc; std::unique_ptr<xrt::kernel> k;
    std::vector<uint32_t> ins; std::unique_ptr<xrt::bo> bI,bA,bC,embB[LM_CHUNKS];
    int8_t*Am; int16_t*Cm; float scales[LM_CHUNKS];
    ...
    bool init(xrt::device&d){
        ...
        bA=std::make_unique<xrt::bo>(d,(size_t)128*1024,XRT_BO_FLAGS_HOST_ONLY,k->group_id(3));
        bC=std::make_unique<xrt::bo>(d,(size_t)128*LM_N*2,XRT_BO_FLAGS_HOST_ONLY,k->group_id(5));
        Am=(int8_t*)bA->map();Cm=(int16_t*)bC->map();
        for(int i=0;i<LM_CHUNKS;i++)embB[i]=std::make_unique<xrt::bo>(d,(size_t)1024*LM_N,XRT_BO_FLAGS_HOST_ONLY,k->group_id(4));
    ...
    void packEmb(){
        ...
        for(int c=0;c<LM_CHUNKS;c++){
            scales[c]=amax/127.0f;
            int n_start=c*LM_N, n_end=std::min((c+1)*LM_N,NV);
            ...
            embB[c]->sync(XCL_BO_SYNC_BO_TO_DEVICE);
        }
    }

    // Run LM head: hidden[1024] → logits[NV]. Returns sample token.
    // Pre-packed B, only sync A in + C out per call.
    int sample(const float*hidden,float*lg_logits){
        ...
        double total_time=0;
        for(int c=0;c<LM_CHUNKS;c++){
            auto r=(*k)((unsigned)3,*bI,(unsigned)ins.size(),*bA,*embB[c],*bC);r.wait();
            bC->sync(XCL_BO_SYNC_BO_FROM_DEVICE);
            float cs=es*scales[c];int n_start=c*LM_N;
            for(int n=0;n<LM_N&&(n_start+n)<NV;n++){float val=(float)Cm[n]*cs;if(!std::isfinite(val))val=0;lg_logits[n_start+n]=val;}
        }

        // Softmax + sample from full logits
        float mx=-1e30f;
        for(int n=0;n<NV;n++){if(lg_logits[n]>mx)mx=lg_logits[n];}
        double sum=0;
        for(int n=0;n<NV;n++){float d=lg_logits[n]-mx;if(d<-80)d=-80;lg_logits[n]=expf(d);sum+=lg_logits[n];}
        float rr=(float)rand()/RAND_MAX*(float)sum,acc=0;
        for(int n=0;n<NV;n++){acc+=lg_logits[n];if(acc>=rr)return n;}
        return NV-1;
    }

    // Get top-K token IDs by softmax probability
    void topk(const float*hidden,float*lg_logits,int*top_ids,int K){
        sample(hidden,lg_logits); // fills lg_logits with softmax values
        struct TI{int id;float v;};TI top[16];for(int b=0;b<K;b++){top[b].id=-1;top[b].v=-1e30f;}
        for(int n=0;n<NV;n++){float v=lg_logits[n];for(int b=0;b<K;b++){if(v>top[b].v){for(int r=K-1;r>b;r--)top[r]=top[r-1];top[b].id=n;top[b].v=v;break;}}}
...
int main(int argc,char**argv){
    ...
    printf("Pre-convert lm_head f32 (untied)...\n");
    {int lm_r,lm_c;float*lm_raw=dequant_i8_to_float(i8p(lo_off),18992,&lm_r,&lm_c);
    ...
    LMCtx lmc;lmc.init(dev);
    printf("LM head: pack emb...\n");lmc.packEmb();
    ...
    std::vector<float>h(H),qo(4096*BS),ko(1024*BS),vo(1024*BS),at(2048*BS),oo(H*BS),gt(6144*BS),su(IM*BS),dwo(H*BS),sb_(H*BS),lg(NV);
    int sp=0; int pt[]={151644,872,198,13048,151645,198,151644,77091,198};
    ...
    // ===== M=16 BATCHED DECODE WITH NPU LM HEAD =====
    printf("=== M=%d Batch + NPU LM head (%d tokens) ===\n",BS,ng);
    auto tgs=std::chrono::steady_clock::now();
    int top_ids[BS], total_accepted=0,n_batches=0;
    double t_boot=0;

    // Boot: single-token decode → NPU LM head → top-16
    {
        auto ts=std::chrono::steady_clock::now();
        float h0[H];memcpy(h0,&h_b[(npt-1)*H],H*4);
        for(int l=0;l<NC;l++){
            memcpy(sb_.data(),h0,H*4);rn_c(h0,in_n[l],H);
            cq.go(l,h0,1,H,5.0f/127.0f,wsc[l].qk,qo.data(),4096);cn(qo.data(),4096);
            ...
            co.go(l,at.data(),1,NH*HD,5.0f/127.0f,wsc[l].o_,oo.data(),H);cn(oo.data(),H);for(int i=0;i<H;i++)h0[i]=sb_[i]+oo[i];
            memcpy(sb_.data(),h0,H*4);rn_c(h0,pa_n[l],H);
            cg.go(l,h0,1,H,5.0f/127.0f,wsc[l].g_,gt.data(),6144);cn(gt.data(),6144);
            for(int i=0;i<IM;i++){float gv=gt[i];if(!std::isfinite(gv))gv=0;su[i]=(gv/(1.0f+expf(-gv)))*gt[IM+i];}
            cd.go(l,su.data(),1,IM,5.0f/127.0f,wsc[l].d_,dwo.data(),H);cn(dwo.data(),H);for(int i=0;i<H;i++)h0[i]=sb_[i]+dwo[i];
        }
        memcpy(sb_.data(),h0,H*4);rn_c(sb_.data(),fin,H);
        auto tlm0=std::chrono::steady_clock::now();
        lmc.topk(sb_.data(),lg.data(),top_ids,BS);
        ...
        printf("  [0] boot=%d lm=%.0fms total=%.0fms\n",top_ids[0],tlm,t_boot);
    }
    ...
    while(step<ng){
        ...
        for(int l=0;l<NC;l++){
            ...
            cq.go(l,h_b.data(),batch_size,H,5.0f/127.0f,wsc[l].qk,qo_b.data(),4096);cn(qo_b.data(),batch_size*4096);
            float*qn=qn_w[l],*kn=kn_w[l];
            ...
            co.go(l,at_b.data(),batch_size,NH*HD,5.0f/127.0f,wsc[l].o_,oo_b.data(),H);cn(oo_b.data(),batch_size*H);
            for(int b=0;b<batch_size;b++)for(int i=0;i<H;i++)h_b[b*H+i]+=oo_b[b*H+i];
            for(int b=0;b<batch_size;b++)rn_c(&h_b[b*H],pa_n[l],H);
            cg.go(l,h_b.data(),batch_size,H,5.0f/127.0f,wsc[l].g_,gt_b.data(),6144);cn(gt_b.data(),batch_size*6144);
            ...
            cd.go(l,su_b.data(),batch_size,IM,5.0f/127.0f,wsc[l].d_,dw_b.data(),H);cn(dw_b.data(),batch_size*H);
        ...
        double batch_ms=std::chrono::duration<double,std::milli>(std::chrono::steady_clock::now()-ts).count();
        printf("  [%d] batch=%d tok=%d lm=%.0fms total=%.0fms (%.0f ms/tok)\n",step,batch_size,top_ids[0],tlm,batch_ms,batch_ms/batch_size);
        step+=batch_size;
    }
