    ...
    for(int l=0;l<NC;l++){
        ...
        for(int pi=0;pi<npt;pi++){
            for(int hh=0;hh<NH;hh++){int kvh=hh/GQA;
                std::vector<float>sc(sp+pi+1);
                for(int p=0;p<sp+pi+1;p++){double s=0;for(int d=0;d<HD;d++)s+=q_batch[pi*NH*HD+hh*HD+d]*kv[l].k[p*NKV*HD+kvh*HD+d];sc[p]=(float)(s/sqrtf(HD));}
                sm(sc.data(),sp+pi+1);
                for(int d=0;d<HD;d++){float s=0;for(int p=0;p<sp+pi+1;p++)s+=sc[p]*kv[l].v[p*NKV*HD+kvh*HD+d];at_batch[pi*NH*HD+hh*HD+d]=s;}
            }
        }
