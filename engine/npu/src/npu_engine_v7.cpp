...
int main(int argc,char**argv){
    ...
    printf("\n=== Single-Token Decode Probe (1 token, 28 layers) ===\n\n");
    {
        ...
        for(int l=0;l<NC;l++){
            ...
            cq.go_probe(l,h.data(),1,H,dynamic_ascale(h.data(),1*H),wsc[l].qk,qo.data(),4096);cn(qo.data(),4096);
            ...
            co.go_probe(l,at.data(),1,NH*HD,dynamic_ascale(at.data(),1*NH*HD),wsc[l].o_,oo.data(),H);cn(oo.data(),H);for(int i=0;i<H;i++)h[i]=sb_[i]+oo[i];
            auto tr0=std::chrono::steady_clock::now();
            memcpy(sb_.data(),h.data(),H*4);rn_c(h.data(),pa_n[l],H);
            cg.go_probe(l,h.data(),1,H,dynamic_ascale(h.data(),1*H),wsc[l].g_,gt.data(),6144);cn(gt.data(),6144);
            for(int i=0;i<IM;i++){float gv=gt[i];if(!std::isfinite(gv))gv=0;su[i]=(gv/(1.0f+expf(-gv)))*gt[IM+i];}
            cd.go_probe(l,su.data(),1,IM,dynamic_ascale(su.data(),1*IM),wsc[l].d_,dwo.data(),H);cn(dwo.data(),H);for(int i=0;i<H;i++)h[i]=sb_[i]+dwo[i];
        ...
        for(int n=0;n<NV;n++){double s=0;const float*e=&emb_f32[(size_t)n*H];for(int k=0;k<H;k++)s+=(double)sb_[k]*e[k];lg[n]=std::isfinite((float)s)?(float)s:-1e30f;if(lg[n]>mx)mx=lg[n];}
        double sum=0;
