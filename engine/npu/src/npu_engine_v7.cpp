...
int main(int argc,char**argv){
    ...
    for(int l=0;l<NC;l++){
        ...
        cq.go_probe(l,h_b.data(),npt,H,5.0f/127.0f,wsc[l].qk,qo_b.data(),4096);cn(qo_b.data(),npt*4096);
        ...
        co.go_probe(l,at_b.data(),npt,NH*HD,5.0f/127.0f,wsc[l].o_,oo_b.data(),H);cn(oo_b.data(),npt*H);
        for(int pi=0;pi<npt;pi++)for(int i=0;i<H;i++)h_b[pi*H+i]+=oo_b[pi*H+i];
        for(int pi=0;pi<npt;pi++)rn_c(&h_b[pi*H],pa_n[l],H);
        cg.go_probe(l,h_b.data(),npt,H,5.0f/127.0f,wsc[l].g_,gt_b.data(),6144);cn(gt_b.data(),npt*6144);
        ...
        cd.go_probe(l,su_b.data(),npt,IM,5.0f/127.0f,wsc[l].d_,dw_b.data(),H);cn(dw_b.data(),npt*H);
    ...
    printf("\n=== Single-Token Decode Probe (1 token, 28 layers) ===\n\n");
    {
        ...
        for(int l=0;l<NC;l++){
            ...
            cq.go_probe(l,h.data(),1,H,5.0f/127.0f,wsc[l].qk,qo.data(),4096);cn(qo.data(),4096);
            ...
            co.go_probe(l,at.data(),1,NH*HD,5.0f/127.0f,wsc[l].o_,oo.data(),H);cn(oo.data(),H);for(int i=0;i<H;i++)h[i]=sb_[i]+oo[i];
            auto tr0=std::chrono::steady_clock::now();
            memcpy(sb_.data(),h.data(),H*4);rn_c(h.data(),pa_n[l],H);
            cg.go_probe(l,h.data(),1,H,5.0f/127.0f,wsc[l].g_,gt.data(),6144);cn(gt.data(),6144);
            for(int i=0;i<IM;i++){float gv=gt[i];if(!std::isfinite(gv))gv=0;su[i]=(gv/(1.0f+expf(-gv)))*gt[IM+i];}
            cd.go_probe(l,su.data(),1,IM,5.0f/127.0f,wsc[l].d_,dwo.data(),H);cn(dwo.data(),H);for(int i=0;i<H;i++)h[i]=sb_[i]+dwo[i];
