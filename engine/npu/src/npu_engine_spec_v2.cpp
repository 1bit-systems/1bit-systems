    ...
    for(int step=0;step<ng;){
        ...
        for(int s=0;s<SPEC&&step+s<ng;s++){
            ...
            for(int l=0;l<DRAFT_L;l++){
                ...
                co.go(l,at.data(),1,NH*HD,5.0f/127.0f,wsc[l].o_,oo.data(),H);cn(oo.data(),H);for(int i=0;i<H;i++)draft_h[s][i]=sb[i]+oo[i];
                ...
                cd.go(l,su.data(),1,IM,5.0f/127.0f,wsc[l].d_,dwo.data(),H);cn(dwo.data(),H);for(int i=0;i<H;i++)draft_h[s][i]=sb[i]+dwo[i];
            }
