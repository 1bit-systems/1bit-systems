...
static uint8_t*md=nullptr; static uint64_t ds=0;
static const char*mh=nullptr; static size_t mhlen=0;
...
static uint64_t fo(const char*h,size_t hl,const char*n){
    ...
    if(o){const char*b=(const char*)memchr(o,'[',e-o);if(b)return strtoull(b+1,nullptr,10);}}p=q+1;}return UINT64_MAX;}
static float*rd(const char*n,int c){if(!mh)return nullptr;uint64_t o=fo(mh,mhlen,n);
    if(o==UINT64_MAX)return nullptr;float*d=static_cast<float*>(std::calloc(c,sizeof(float)));uint16_t*s=(uint16_t*)(md+ds+o);for(int i=0;i<c;i++)d[i]=bf(s[i]);return d;}
static int load(const char*p){
    int f=open(p,O_RDONLY);struct stat st;fstat(f,&st);md=(uint8_t*)mmap(nullptr,st.st_size,PROT_READ,MAP_PRIVATE,f,0);close(f);
...
static void* drain(void*a){
    int fd=*(int*)a;std::free(a);int m=1048576;fcntl(fd,F_SETPIPE_SZ,&m);
    ...
    close(fd);close(resp_w);return nullptr;}
static void* rdyst(void*a){
    int fd=*(int*)a;std::free(a);char buf[4096];FILE*f=fdopen(fd,"r");
    if(!f)return nullptr;while(fgets(buf,sizeof(buf),f))if(strstr(buf,"WORKER_READY"))break;fclose(f);return nullptr;}
...
static int spawn(const char*mo,const char*ta){
    ...
    wp=fork();
    if(wp<0)return -1;
    if(wp==0){close(tc[1]);dup2(tc[0],0);close(tc[0]);close(fc[0]);dup2(fc[1],1);close(fc[1]);close(ec[0]);dup2(ec[1],2);close(ec[1]);close(rp[0]);close(rp[1]);execlp(bin,bin,mo,"--model-tag",ta,"--worker",nullptr);_exit(1);}
    close(tc[0]);close(fc[1]);close(ec[1]);wsi=tc[1];wso=fc[0];
    int*df=static_cast<int*>(std::malloc(sizeof(int)));*df=fc[0];pthread_t dt;
    if(pthread_create(&dt,nullptr,drain,df)!=0){std::free(df);close(fc[0]);}
    else pthread_detach(dt);
    int*ef=static_cast<int*>(std::malloc(sizeof(int)));*ef=ec[0];pthread_t et;
    if(pthread_create(&et,nullptr,rdyst,ef)!=0){std::free(ef);close(ec[0]);}
...
static void cn(float*x,int n){for(int i=0;i<n;i++)if(!std::isfinite(x[i]))x[i]=0;}
...
static float*cc,*sc;
static void mk_rope(int ms){cc=static_cast<float*>(std::calloc((size_t)ms*HD,sizeof(float)));sc=static_cast<float*>(std::calloc((size_t)ms*HD,sizeof(float)));int h2=HD/2;
...
static int fwd(int tok){
    ...
    float*lg=static_cast<float*>(std::malloc((size_t)NV*sizeof(float)));
    #pragma omp parallel for
    for(int n=0;n<NV;n++){double s=0;for(int k=0;k<H;k++)s+=(double)h[k]*lme[(size_t)n*H+k];lg[n]=(float)s;}
    int bst=0;float bv=lg[0];for(int n=1;n<NV;n++)if(lg[n]>bv){bv=lg[n];bst=n;}std::free(lg);tp++;return bst;}
...
static void*hc(void*a){
    int fd=(int)(long)a;char b[65536];ssize_t n=read(fd,b,sizeof(b)-1);if(n<=0){close(fd);return nullptr;}b[n]=0;
    char m[16],p[256];if(sscanf(b,"%15s %255s",m,p)<2){close(fd);return nullptr;}
    if(strcmp(m,"GET")==0){sj(fd,"{\"ready\":%d}",ready);return nullptr;}
...
int main(int argc,char**argv){
    ...
    for(int l=0;l<NC&&l<256;l++){kc[l]=static_cast<float*>(std::calloc((size_t)4096*NKV*HD,sizeof(float)));vc[l]=static_cast<float*>(std::calloc((size_t)4096*NKV*HD,sizeof(float)));}
    mk_rope(4096);
    ...
    while(1){struct sockaddr_in ca;socklen_t cl=sizeof(ca);int cf=accept(fd,(struct sockaddr*)&ca,&cl);
        if(cf<0)break;pthread_t t;pthread_create(&t,nullptr,hc,(void*)(long)cf);pthread_detach(t);}
    return 0;}
