...
// from that set proportionally.
static int sample_top_k_top_p(const float* logits, int n, int top_k, float top_p, float temperature) {
    ...
    for (int i = 0; i < cutoff; i++) {
        acc += probs[i];
        if (acc >= rr) return scored[i].second;
    }
...
struct I8Ctx{const char*name;int MD,KD,ND;std::unique_ptr<xrt::xclbin>xc;std::unique_ptr<xrt::hw_context>hc;std::unique_ptr<xrt::kernel>k;std::vector<uint32_t>ins;std::unique_ptr<xrt::bo>bI,bA,bC,layerB[NC];int8_t*Am;int32_t*Cm;std::vector<float>col_scale;
...
void packB(int l,const float*w,int K,int N,float&sout){float amax=0;col_scale.resize(N);for(int n=0;n<N;n++){float camax=0;for(int k=0;k<K;k++){float a=fabsf(w[(size_t)k*N+n]);if(std::isfinite(a)&&a>camax)camax=a;}if(camax>amax)amax=camax;col_scale[n]=camax>1e-12f?camax/127.0f:1.0f;}sout=amax/127.0f;auto*Bm=(int8_t*)layerB[l]->map();for(int n=0;n<N;n++){float is=127.0f/(col_scale[n]*127.0f);for(int k=0;k<K;k++){float v=w[(size_t)k*N+n];if(!std::isfinite(v))v=0;int x=(int)roundf(v/col_scale[n]);if(x>127)x=127;else if(x<-127)x=-127;Bm[(size_t)k*N+n]=(int8_t)x;}}layerB[l]->sync(XCL_BO_SYNC_BO_TO_DEVICE);}
// Per-column INT8 quantization: each output column has its own scale (~4.6x error reduction)
static void packB_per_col(std::vector<int8_t>& out,const float*w,int K,int N,std::vector<float>& sc){
sc.resize(N);out.resize((size_t)K*N);
for(int n=0;n<N;n++){float amax=0;for(int k=0;k<K;k++){float a=fabsf(w[(size_t)k*N+n]);if(std::isfinite(a)&&a>amax)amax=a;}
...
int main(int argc, char** argv) {
    ...
    while (std::getline(std::cin, line)) {
        ...
        for (int step = 0; step < max_new; step++) {
            ...
            int tok = sample_top_k_top_p(lg.data(), NV, TOP_K, TOP_P, TEMPERATURE);
