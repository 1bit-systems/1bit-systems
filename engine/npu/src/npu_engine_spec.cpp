...
int main(int argc,char**argv){
    ...
    for(int l=0;l<NC;l++){
        ...
        // Batched attention
        for(int pi=0;pi<npt;pi++){for(int hh=0;hh<NH;hh++){int kvh=hh/GQA;int pl=sp+pi+1;std::vector<float>sc(cl);
            for(int p=0;p<pl;p++){double s=0;for(int d=0;d<HD;d++)s+=qo_b[pi*NH*HD+hh*HD+d]*kv[l].k[p*NKV*HD+kvh*HD+d];sc[p]=(float)(s/sqrtf(HD));}
            sm(sc.data(),pl);for(int d=0;d<HD;d++){float s=0;for(int p=0;p<pl;p++)s+=sc[p]*kv[l].v[p*NKV*HD+kvh*HD+d];at_b[pi*NH*HD+hh*HD+d]=s;}}}
    ...
    for(int step=0;step<ng;){
        ...
        for(int s=0;s<SPEC&&step+s<ng;s++){
            ...
            for(int l=0;l<NC;l++){
                ...
                // Minimal — skip KVCache write during draft (use scratch)
                ...
                // Quick softmax + O + MLP
                for(int w=0;w<AW;w++){float*Qw=&qo[w*WQH*HD];for(int hh=0;hh<WQH;hh++)memset(&at[hh*HD],0,HD*4);}
                co.go(l,at.data(),1,NH*HD,dynamic_ascale(at.data(),NH*HD),wsc[l].o_,oo.data(),H);
                ...
                cd.go(l,su.data(),1,IM,dynamic_ascale(su.data(),IM),wsc[l].d_,dwo.data(),H);
