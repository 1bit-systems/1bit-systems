...
int main(int argc,char**argv){
    ...
    for(int l=0;l<NC;l++){
        ...
        // Batched attention
        for(int pi=0;pi<npt;pi++){for(int hh=0;hh<NH;hh++){int kvh=hh/GQA;std::vector<float>sc(cl);
            for(int p=0;p<cl;p++){double s=0;for(int d=0;d<HD;d++)s+=qo_b[pi*NH*HD+hh*HD+d]*kv[l].k[p*NKV*HD+kvh*HD+d];sc[p]=(float)(s/sqrtf(HD));}
            sm(sc.data(),cl);for(int d=0;d<HD;d++){float s=0;for(int p=0;p<cl;p++)s+=sc[p]*kv[l].v[p*NKV*HD+kvh*HD+d];at_b[pi*NH*HD+hh*HD+d]=s;}}}
    ...
    for(int step=0;step<ng;){
        ...
        for(int s=0;s<SPEC&&step+s<ng;s++){
            ...
            for(int l=0;l<NC;l++){
                ...
                cg.go(l,draft_h[s],1,H,5.0f/127.0f,wsc[l].g_,gt.data(),6144);cn(gt.data(),6144);
