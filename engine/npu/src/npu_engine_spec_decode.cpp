...
static int lm_head(const float*h,const float*lm_f32,const float*fn,int V,float*lg,float*sb){
    memcpy(sb,h,H*4);rn_c(sb,fn,H);
    for(int n=0;n<V;n++){double s=0;const float*e=&lm_f32[(size_t)n*H];for(int k=0;k<H;k++)s+=(double)sb[k]*e[k];lg[n]=std::isfinite((float)s)?(float)s:-1e30f;}
    for(int i=0;i<V;i++)if(!std::isfinite(lg[i]))lg[i]=-1e30f;
    ...
    for(int step=0;step<ng;){
        ...
        for(int s=0;s<SPEC&&step+s<ng;s++){
            ...
            for(int l=0;l<NC;l++){
                ...
                // Q/K norm + RoPE, write K/V to cache at draft_sp
                float*qn=qn_w[l],*kn=kn_w[l];
                ...
                for(int hh=0;hh<NH;hh++){double sq=0;for(int d=0;d<HD;d++)sq+=qo[hh*HD+d]*qo[hh*HD+d];float iq=1.0f/sqrtf((float)(sq/HD)+EPS);for(int d=0;d<HD;d++)qo[hh*HD+d]*=iq*qn[d];ra(&qo[hh*HD],HD,sp+s);
                    if(hh%GQA==0){int kvh=hh/GQA;double sk=0;for(int d=0;d<HD;d++)sk+=ko[kvh*HD+d]*ko[kvh*HD+d];float ik=1.0f/sqrtf((float)(sk/HD)+EPS);for(int d=0;d<HD;d++)ko[kvh*HD+d]*=ik*kn[d];ra(&ko[kvh*HD],HD,sp+s);memcpy(&kv[l].k[(sp+s)*NKV*HD+kvh*HD],&ko[kvh*HD],HD*4);memcpy(&kv[l].v[(sp+s)*NKV*HD+kvh*HD],&vo[kvh*HD],HD*4);}}
                kv[l].n=sp+s+1;int cl=kv[l].n;
                // Full attention against cache (main + draft up to draft_sp)
                ...
                cg.go(l,draft_h[s],1,H,dynamic_ascale(draft_h[s],1*H),wsc[l].g_,gt_b.data(),6144);cn(gt_b.data(),6144);
            ...
            draft_tokens[s]=lm_head(draft_h[s],lm_head_f32.data(),fin,NV,lg.data(),sb.data());
            drafted_total++;
        }
        ...
        for(int l=0;l<NC;l++){
            ...
            for(int i=0;i<IM;i++){float gv=gt_b[i];if(!std::isfinite(gv))gv=0;su_b[i]=(gv/(1.0f+expf(-gv)))*gt_b[IM+i];}
            cd.go(l,su_b.data(),1,IM,dynamic_ascale(su_b.data(),1*IM),wsc[l].d_,dw_b.data(),H);cn(dw_b.data(),H);for(int i=0;i<H;i++)h[i]=sb[i]+dw_b[i];
        }
        sp++;
        ...
        for(int s=0;s<SPEC&&(step-1+s)<ng;s++){
            int check_tok=lm_head(draft_h[s],lm_head_f32.data(),fin,NV,lg.data(),sb.data());
