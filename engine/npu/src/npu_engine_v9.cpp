 ...
 *  v3→v6→v8→v9: 244→50→27→ ? ms/tok. Target: <20ms/tok effective. */
#include <cstdio>
#include <cstdlib>
#include <cstring>
#include <cmath>
...
int main(int argc,char**argv){
    ...
    printf("=== NPU Engine v9 — M=%d Batch Decode (M=%d) ===\n\n",BS,npt+1);
    ...
    // Boot: single-token decode → top-16 token IDs
    {
        ...
        printf("  [0] boot=%d top16 first 8: %d,%d,%d,%d,%d,%d,%d,%d (%.0fms)\n",
    ...
    printf("\n=== %.1f ms/tok effective (boot=%.0fms, batches=%d, accepted=%d) ===\n",
        tts*1000/ng,t_boot,n_batches,total_accepted);
