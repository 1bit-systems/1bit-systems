/** NPU Engine v8 — M=8 batch decode. Amortize 1334μs NPU compute across 8 tokens.
 *  Single-token boot, then chain 8-token batches. Target: <30ms/tok effective. */
#include <cstdio>
#include <cstdlib>
#include <cstring>
#include <cmath>
...
static constexpr float EPS=1e-6f; static constexpr int XM=128, BS=8;
...
// LM head: top-K sampling. lg buf reused across calls. Returns top-K token IDs by logit.
inline void lm_topk(const float*hidden,float*lg,int*top_ids,int K){
    float mx=-1e30f;
    ...
    float r=(float)rand()/RAND_MAX*(float)sum,acc=0;int sampled=-1;
    for(int n=0;n<NV;n++){acc+=lg[n];if(acc>=r&&sampled<0){sampled=n;top_ids[0]=n;break;}}
    // Top-K by value (already have lg in memory with softmax applied)
    struct TI{int id;float v;};TI top[8];for(int b=0;b<K;b++){top[b].id=-1;top[b].v=-1e30f;}
...
// LM head: process BS hidden states sequentially, get top-8 from first, fill batch token IDs.
// Each hidden state pass = NV×H dot products = 155M fma. OpenMP on 16 cores = ~6ms/pass.
inline void lm_batch_topk(const float*hidden_bs,float*lg,int*top_ids){
    lm_topk(hidden_bs,lg,top_ids,BS);
}

int main(int argc,char**argv){
    setvbuf(stdout,NULL,_IONBF,0);
    int npt=9,ng=(argc>1)?atoi(argv[1]):16;
    printf("=== NPU Engine v8 — M=%d Batch Decode (M=%d) ===\n\n",BS,npt+1);
    ...
    // Batched buffers: XM rows (XM=128, way more than BS=8)
    ...
    double t_boot=0,t_batch_lm=0;
    int n_batches=0;

    // Step 0: Single-token boot to get initial hidden + top-BS token IDs
    {
        ...
        printf("  [0] boot=%d top8=%d,%d,%d,%d,%d,%d,%d,%d (%.0fms)\n",
            top_ids[0],top_ids[0],top_ids[1],top_ids[2],top_ids[3],top_ids[4],top_ids[5],top_ids[6],top_ids[7],t_boot);
    }

    // Main loop: BS-token batched forward pass → top-K from batch[0] hidden → next batch
    int step=1;
    while(step<ng){
        auto ts=std::chrono::steady_clock::now();
        ...
        // Batched forward pass (28 layers, BS tokens)
        for(int l=0;l<NC;l++){
    ...
    printf("\n=== %.0f ms/tok effective (boot=%.0fms, accepted=%d, batches=%d) ===\n",
        tts*1000/ng,t_boot,total_accepted,n_batches);
