...
//! macOS → Metal. Linux → Vulkan by default (⭐ primary), or CUDA with
//!   `-Dbackend=cuda` (NVIDIA / WSL2), or ROCm with `-Dbackend=rocm` (AMD HIP).
//! See docs/cuda-backend.md.
...
/// True when compiling for Linux with `-Dbackend=rocm` (AMD GPU via HIP).
pub const is_rocm = builtin.os.tag == .linux and std.mem.eql(u8, build_options.backend, "rocm");
/// True when compiling for Linux with the Vulkan backend (the Linux default — ⭐ primary).
pub const is_vulkan = builtin.os.tag == .linux and !is_cuda and !is_rocm;

// Backend-specific module imports, resolved at comptime.
// Only the active backend's code is compiled.
/// Platform-specific GPU device module (Metal / ROCm / CUDA / Vulkan).
pub const backend = if (is_metal)
    @import("../metal/device.zig")
else if (is_rocm)
    @import("../rocm/device.zig")
else if (is_cuda)
...
/// Device buffer allocation + H2D/D2H staging (ROCm / CUDA / Vulkan).
pub const buffer_mod = if (is_rocm)
    @import("../rocm/buffer.zig")
else if (is_cuda)
    ...
    struct {};
/// Compute pipeline creation (ROCm HIPRTC / CUDA NVRTC / Vulkan).
pub const pipeline_mod = if (is_rocm)
    @import("../rocm/pipeline.zig")
else if (is_cuda)
    ...
    struct {};
/// Command/stream recording (ROCm streams+events / CUDA streams+events / Vulkan command buffers).
pub const command_mod = if (is_rocm)
    @import("../rocm/command.zig")
else if (is_cuda)
    ...
    struct {};
/// GPU detection / tuning (Vulkan only for now; ROCm/CUDA caps come from device modules).
...
test "backend selection is correct for this platform" {
    if (builtin.os.tag == .macos) {
        ...
        // Vulkan is the default; ROCm/CUDA are opt-in backends
        try std.testing.expect(is_vulkan);
        try std.testing.expect(!is_metal);
        try std.testing.expect(!is_rocm);
        try std.testing.expect(!is_cuda);
    }
}
