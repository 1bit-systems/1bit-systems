...
class NpuFlmBackend : public InferenceBackend {
    ...
    std::string running_model_ = "qwen3:0.6b";     // actual FLM tag loaded
    std::string running_backend_label_ = "NPU FLM"; // descriptive label for response headers
...
public:
    ...
    const char* name() const override { return running_backend_label_.c_str(); }
    ...
    bool load_model(const ModelConfig& cfg) override {
        ...
        std::string req_model = cfg.model_name.empty() ? "<unknown>" : cfg.model_name;
        ...
        fprintf(stderr, "  " "\033[33mWARN: requested model \"%s\" (H=%d) mapping to FLM %s\033[0m\n",
                req_model.c_str(), cfg.hidden_size, model_tag_.c_str());

        // Store what's actually running for response headers
        running_model_ = model_tag_;
        running_backend_label_ = std::string("NPU FLM (") + model_tag_ + std::string(") fallback from ") + req_model;
