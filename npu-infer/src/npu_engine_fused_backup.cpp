                                              ...
                                              int XK, int XN) // xclbin dims
{
    ...
    for (int k = 0; k < in_feat && k < XK; k++)
        for (int n = 0; n < out_feat && n < XN; n++) {
            ...
            if (std::isfinite(v)) Bf[k * XN + n] = v * 16.0f;  // logit_scale
        }
                          ...
                          int GQA, const uint8_t *d, uint64_t df)
{
    ...
    if (hn) printf("  L%d: NAN in h BEFORE Q!\n", l);
    ...
    for (int hh = 0; hh < NH; hh++) {
        // Q norm
        double sq = 0;
        for (int d = 0; d < HD; d++) sq += qo[hh * HD + d];
        ...
        if (hh % GQA == 0) {
            int kv = hh / GQA;
            double sk = 0;
            for (int d = 0; d < HD; d++) sk += ko[kv * HD + d] * ko[kv * HD + d];
    ...
    for (int hh = 0; hh < NH; hh++) {
        ...
        if (mx > 100.0f && l >= 19) printf("  L%d hh=%d: mx=%.1f cl=%d sc[0]=%.1f\n", l, hh, mx, cl, sc[0]);
        double sum = 0;
        for (int p = 0; p < cl; p++) { sc[p] = expf(sc[p] - mx); sum += sc[p]; }
        if (sum == 0.0 && l >= 19) printf("  L%d hh=%d: ZERO SUM! mx=%.1f\n", l, hh, mx);
