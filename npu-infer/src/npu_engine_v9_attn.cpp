/** NPU Engine v9 — Chess GEMM + Activation Scaling + NPU Attention + Temp sampling */
#include <cstdio>
#include <cstdlib>
#include <cstring>
#include <cmath>
...
static constexpr float EPS=1e-6f,TEMP=0.8f; // Higher temperature for diversity

// GEMM context (swap-based, BO-cached)
struct ChessCtx {
    ...
    std::unique_ptr<xrt::bo> bI,bA,bB,bC;std::vector<float> Ab,Cb;bool alive=false;
    ~ChessCtx(){release();}void release(){k.reset();hc.reset();alive=false;}
    bool init(xrt::device& d){
        release();dev=&d;FILE*f=fopen(ip,"rb");if(!f)return false;
        ...
        if(!bI){bI=std::make_unique<xrt::bo>(d,ins.size()*4,XCL_BO_FLAGS_CACHEABLE,k->group_id(1));
    ...
    bool sa(xrt::device& d,ChessCtx*& a){if(alive){a=this;return true;}if(a&&a!=this)a->release();a=this;return init(d);}
    void go(const float*A,int am,int ak,const uint8_t*B,size_t Bsz,float*C,int an){
        ...
        auto As=gemm_atb::layout_A_L1_2x1_8x8block(Ab,MD,KD,MT,KT);uint16_t*Ab2=(uint16_t*)bA->map();
        ...
        bC->sync(XCL_BO_SYNC_BO_FROM_DEVICE);uint16_t*Cc=(uint16_t*)bC->map();int MN=MD*ND;
...
static uint64_t jo(const char*js,size_t jl,const char*nm){
    size_t nl=strlen(nm);const char*p=js,*e=js+jl;while(p<e){auto q=(const char*)memmem(p,e-p,nm,nl);if(!q)return 0;
        if(q>js&&*(q-1)=='"'&&*(q+nl)=='"'){auto o=strstr(q,"\"data_offsets\"");if(o){auto a=strchr(o,'[');if(a)return strtoull(a+1,NULL,10);}}p=q+1;}return 0;}

int main(){
    setvbuf(stdout,NULL,_IONBF,0);
    printf("=== v9 Activation-Scaled + NPU Attn + Temp=%.1f ===\n\n",TEMP);
    ...
    printf("Init...\n");xrt::device dev(0);
    #define BB "chess_infer" /* set $NPU_CHESS_DIR */
    ChessCtx cq{"QKV",BB"/qkv/qwen3_qkv_chess.xclbin",BB"/qkv/qwen3_qkv_chess.insts",XM,H,4096};
    ChessCtx co{"O",  BB"/o/qwen3_o_chess.xclbin",    BB"/o/qwen3_o_chess.insts",    XM,2048,H};
    ChessCtx cg{"GU", BB"/gu/qwen3_gu_chess.xclbin",  BB"/gu/qwen3_gu_chess.insts",  XM,H,6144};
    ChessCtx cd{"D",  BB"/d/qwen3_d_chess.xclbin",    BB"/d/qwen3_d_chess.insts",    XM,IM,H};
    ChessCtx* active=nullptr;cq.sa(dev,active);
    ...
    for(int l=0;l<NC;l++){
        ...
        std::vector<float>w((size_t)(qr+kr+vr)*H);memcpy(w.data(),qw,qr*H*4);memcpy(w.data()+qr*H,kw,kr*H*4);memcpy(w.data()+(qr+kr)*H,vw,vr*H*4);
        ...
        std::vector<float>w2((size_t)(gr+ur)*H);memcpy(w2.data(),gw,gr*H*4);memcpy(w2.data()+gr*H,uw,ur*H*4);
    ...
    std::vector<KV>kv(NC);
    std::vector<float>h(H),qo(4096),ko(1024),vo(1024),at(2048),oo(H),gt(6144),u(IM),su(IM),dw(H),lg(NV),sb(H);

    // Layer with activation scaling: boost activations to ~1.0 before each GEMM
    auto run_layer=[&](int l){
        memcpy(sb.data(),h.data(),H*4);
        rn(h.data(),(const uint16_t*)(md+df+lo[l].in_off),H);
        // Scale activations to use full BFP16 range (~1.0 instead of ~0.03)
        float s_qkv=32.0f;for(int i=0;i<H;i++)h[i]*=s_qkv;
        cq.sa(dev,active);cq.go(h.data(),1,H,pw[l].qk.data(),pw[l].qk.size(),qo.data(),4096);
        for(int i=0;i<4096;i++)qo[i]/=s_qkv;for(int i=0;i<H;i++)h[i]/=s_qkv;
        cn(qo.data(),4096);memcpy(ko.data(),&qo[2048],1024*4);memcpy(vo.data(),&qo[3072],1024*4);
        ...
        // Scale atto for O GEMM
        float s_o=32.0f;for(int i=0;i<NH*HD;i++)at[i]*=s_o;
        co.sa(dev,active);co.go(at.data(),1,NH*HD,pw[l].o_.data(),pw[l].o_.size(),oo.data(),H);
        for(int i=0;i<H;i++)oo[i]/=s_o;for(int i=0;i<NH*HD;i++)at[i]/=s_o;
        cn(oo.data(),H);for(int i=0;i<H;i++)h[i]=sb[i]+oo[i];
        memcpy(sb.data(),h.data(),H*4);rn(h.data(),(const uint16_t*)(md+df+lo[l].pa_off),H);
        // Scale for GU GEMM
        float s_gu=32.0f;for(int i=0;i<H;i++)h[i]*=s_gu;
        cg.sa(dev,active);cg.go(h.data(),1,H,pw[l].g_.data(),pw[l].g_.size(),gt.data(),6144);
        for(int i=0;i<6144;i++)gt[i]/=s_gu;for(int i=0;i<H;i++)h[i]/=s_gu;
        cn(gt.data(),6144);memcpy(u.data(),&gt[IM],IM*4);
        for(int i=0;i<IM;i++)su[i]=(gt[i]/(1.0f+expf(-gt[i])))*u[i];
        // Scale for D GEMM
        float s_d=32.0f;for(int i=0;i<IM;i++)su[i]*=s_d;
        cd.sa(dev,active);cd.go(su.data(),1,IM,pw[l].d_.data(),pw[l].d_.size(),dw.data(),H);
        for(int i=0;i<H;i++)dw[i]/=s_d;for(int i=0;i<IM;i++)su[i]/=s_d;
        cn(dw.data(),H);for(int i=0;i<H;i++)h[i]=sb[i]+dw[i];
    };

    int pt[]={151643,872,198,11852,151644,198,151643,77091,198};int npt=9,sp=0;
    printf("=== Prefill %d ===\n",npt);
    for(int pi=0;pi<npt;pi++){for(int i=0;i<H;i++)h[i]=bf16_g(emb[pt[pi]*H+i]);for(int l=0;l<NC;l++)run_layer(l);sp++;if(pi%3==0)printf("  %d/%d\n",pi+1,npt);}
    ...
    for(int st=0;st<ng;st++){auto ts=std::chrono::steady_clock::now();
        for(int l=0;l<NC;l++)run_layer(l);
        ...
        printf("  [%d] %d (%.0fms) hn=%.2f [%.1f,%.1f]\n",st,tok,ms,fn2(h.data(),H),mn,mx);
