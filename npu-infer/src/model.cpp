...
int npu_dequant_block(void* out, const void* in,
    ...
    std::memset(bf16_out, 0, num_rows * block_cols * 2);
    ...
    for (int r = 0; r < num_rows; r++) {
        for (int c = 0; c < num_cols; c++) {
            ...
            uint16_t bf16_val;
            std::memcpy(&bf16_val, &data[i8_byte_idx], 2);
...
int npu_pack_weight_bo(uint8_t* bo_buffer, const void* in,
    ...
    std::memset(bo_buffer, 0, bo_size);
...
ModelWeights* model_load(const char* path, ModelConfig config) {
    ModelWeights* mw = static_cast<ModelWeights*>(std::calloc(1, sizeof(ModelWeights)));
    if (!mw) return nullptr;
    
    std::memcpy(&mw->config, &config, sizeof(config));
    ...
    mw->file_data = static_cast<uint8_t*>(::mmap(nullptr, mw->file_size, PROT_READ, MAP_PRIVATE, fd, 0));
    ::close(fd);
    
    if (mw->file_data == MAP_FAILED) {
        LOG_ERROR("mmap failed: %s", std::strerror(errno));
        std::free(mw);
        return nullptr;
    }
    
    uint64_t header_size;
    std::memcpy(&header_size, mw->file_data, 8);
    ...
    int max_tensors = 512;
    TensorDesc* tensors = static_cast<TensorDesc*>(std::calloc(max_tensors, sizeof(TensorDesc)));
    ...
    if (idx_emb >= 0) std::memcpy(&mw->embed_tokens, &tensors[idx_emb], sizeof(TensorDesc));
    
    // Allocate per-layer weights
    mw->layers = static_cast<LayerWeights*>(std::calloc(config.num_layers, sizeof(LayerWeights)));
    ...
    for (int l = 0; l < config.num_layers; l++) {
        ...
        if (idx >= 0) std::memcpy(&layer->input_layernorm_weight, &tensors[idx], sizeof(TensorDesc));
        
        std::snprintf(name_buf, sizeof(name_buf),
                 "model.layers.%d.post_attention_layernorm.weight", l);
        idx = find_tensor(name_buf, tensors, num_tensors);
        if (idx >= 0) std::memcpy(&layer->post_attention_layernorm_weight, &tensors[idx], sizeof(TensorDesc));
        
        std::snprintf(name_buf, sizeof(name_buf),
                 "model.layers.%d.self_attn.q_norm.weight", l);
        idx = find_tensor(name_buf, tensors, num_tensors);
        if (idx >= 0) std::memcpy(&layer->q_norm_weight, &tensors[idx], sizeof(TensorDesc));
        
        std::snprintf(name_buf, sizeof(name_buf),
                 "model.layers.%d.self_attn.k_norm.weight", l);
        idx = find_tensor(name_buf, tensors, num_tensors);
        if (idx >= 0) std::memcpy(&layer->k_norm_weight, &tensors[idx], sizeof(TensorDesc));
        
        std::snprintf(name_buf, sizeof(name_buf),
                 "model.layers.%d.self_attn.q_proj.weight", l);
        idx = find_tensor(name_buf, tensors, num_tensors);
        if (idx >= 0) std::memcpy(&layer->q_proj_weight, &tensors[idx], sizeof(TensorDesc));
        
        std::snprintf(name_buf, sizeof(name_buf),
                 "model.layers.%d.self_attn.k_proj.weight", l);
        idx = find_tensor(name_buf, tensors, num_tensors);
        if (idx >= 0) std::memcpy(&layer->k_proj_weight, &tensors[idx], sizeof(TensorDesc));
        
        std::snprintf(name_buf, sizeof(name_buf),
                 "model.layers.%d.self_attn.v_proj.weight", l);
        idx = find_tensor(name_buf, tensors, num_tensors);
        if (idx >= 0) std::memcpy(&layer->v_proj_weight, &tensors[idx], sizeof(TensorDesc));
        
        std::snprintf(name_buf, sizeof(name_buf),
                 "model.layers.%d.self_attn.o_proj.weight", l);
        idx = find_tensor(name_buf, tensors, num_tensors);
        if (idx >= 0) std::memcpy(&layer->o_proj_weight, &tensors[idx], sizeof(TensorDesc));
        
        std::snprintf(name_buf, sizeof(name_buf),
                 "model.layers.%d.mlp.gate_proj.weight", l);
        idx = find_tensor(name_buf, tensors, num_tensors);
        if (idx >= 0) std::memcpy(&layer->gate_proj_weight, &tensors[idx], sizeof(TensorDesc));
        
        std::snprintf(name_buf, sizeof(name_buf),
                 "model.layers.%d.mlp.up_proj.weight", l);
        idx = find_tensor(name_buf, tensors, num_tensors);
        if (idx >= 0) std::memcpy(&layer->up_proj_weight, &tensors[idx], sizeof(TensorDesc));
        
        std::snprintf(name_buf, sizeof(name_buf),
                 "model.layers.%d.mlp.down_proj.weight", l);
        idx = find_tensor(name_buf, tensors, num_tensors);
        if (idx >= 0) std::memcpy(&layer->down_proj_weight, &tensors[idx], sizeof(TensorDesc));
    }
    
    // Final norm
    int idx_fn = find_tensor("model.norm.weight", tensors, num_tensors);
    if (idx_fn >= 0) std::memcpy(&mw->norm_weight, &tensors[idx_fn], sizeof(TensorDesc));
    
    // LM head
    int idx_lm = find_tensor("lm_head.weight", tensors, num_tensors);
    if (idx_lm >= 0) std::memcpy(&mw->lm_head_weight, &tensors[idx_lm], sizeof(TensorDesc));
...
static int parse_json_metadata(const uint8_t* json_data, uint64_t json_len,
    ...
    while (p < end && count < max_tensors) {
        ...
        bool ends_with_weight = (key_len > 7 && std::memcmp(key_end - 7, ".weight", 7) == 0);
        bool is_lm_head = (key_len == 12 && std::memcmp(key_str, "lm_head.weight", 12) == 0);
        ...
        const char* dtype_pos = std::strstr(p, "\"dtype\"");
        if (dtype_pos) {
            const char* val_start = std::strchr(dtype_pos, ':');
            if (val_start) {
                val_start++;
                ...
                std::memcpy(t->dtype, val_start, copy_len);
                t->dtype[copy_len] = '\0';
            }
        }
        
        const char* shape_pos = std::strstr(p, "\"shape\"");
        if (shape_pos) {
            const char* arr_start = std::strchr(shape_pos, '[');
            if (arr_start) {
                arr_start++;
                t->ndim = 0;
                ...
                while (*sp != ']' && sp < end && t->ndim < 4) {
                    while (*sp == ' ' || *sp == ',') sp++;
                    if (*sp >= '0' && *sp <= '9') {
                        t->shape[t->ndim] = std::strtol(sp, (char**)&sp, 10);
                        t->ndim++;
                    } else break;
                }
            }
        ...
        const char* off_pos = std::strstr(p, "\"data_offsets\"");
        if (off_pos) {
            const char* arr_start = std::strchr(off_pos, '[');
            if (arr_start) {
                ...
                while (*sp != ']' && sp < end && off_count < 2) {
                    while (*sp == ' ' || *sp == ',') sp++;
                    if (*sp >= '0' && *sp <= '9') {
                        offsets[off_count] = std::strtoull(sp, (char**)&sp, 10);
                        off_count++;
                    } else break;
                }
                if (off_count == 2) {
...
static int find_tensor(const char* name, TensorDesc* tensors, int count) {
    for (int i = 0; i < count; i++) {
        ...
        if (std::strncmp(tensors[i].name, name, nlen) == 0 &&
            std::strlen(tensors[i].name) == nlen) {
            return i;
        }
    }
    return -1;
