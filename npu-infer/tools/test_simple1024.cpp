...
int main(){
    ...
    const char*xp="final_1024x1024x2048_128x64x128.xclbin";
    const char*ip="insts_1024x1024x2048_128x64x128.txt";
    ...
    // A all 1.0, B all 2.0 → C = K*2 = 2048 everywhere
    ...
    int err=0;
    for(int i=0;i<M*N;i++){float v=bf16f(cb[i]);if(fabs(v-2048.0f)>2.0f){if(err<5)printf("  [%d]=%.0f\n",i,v);err++;}}
    printf("  Expected: 2048  Errors: %d / %d\n",err,M*N);
    return err?1:0;
}
