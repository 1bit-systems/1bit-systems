...
class GPUBackend:
    """GPU inference via llama-server with Vulkan backend running on Radeon 8060S."""
    LLAMA_SERVER_BIN = os.environ.get(
        "GPU_LLAMA_SERVER",
        "/home/bcloud/.cache/lemonade/bin/llamacpp/vulkan/llama-server"
    )
    LLAMA_LD_PATH = os.environ.get(
        "GPU_LLAMA_LD_PATH",
        "/home/bcloud/.cache/lemonade/bin/llamacpp/vulkan"
    )
    DEFAULT_MODEL = os.environ.get(
        "GPU_MODEL",
        "/home/bcloud/models/bonsai-1.7b/Bonsai-1.7B-F16.gguf"
    )
    ...
    def start(self):
        if not os.path.exists(self.LLAMA_SERVER_BIN):
            print(f"  \u26a0\ufe0f  GPU llama-server not found at {self.LLAMA_SERVER_BIN}")
            return
        if not os.path.exists(self.DEFAULT_MODEL):
            print(f"  \u26a0\ufe0f  GPU model not found at {self.DEFAULT_MODEL}")
            return
        print(f"  Starting GPU backend (llama-server Vulkan) on port {self.port}...")
        env = os.environ.copy()
        env["LD_LIBRARY_PATH"] = self.LLAMA_LD_PATH + ":" + env.get("LD_LIBRARY_PATH", "")
        self.process = subprocess.Popen(
        ...
        if self.process and self.process.poll() is None:
            ...
            print(f"  \u26a0\ufe0f  GPU backend failed to start")
            self.process = None
...
class CPUBackend:
    """CPU inference via llama-server (no GPU offloading).
    Uses the same binary as GPU but with -ngl 0 (CPU only).
    Falls back to a smaller model for faster startup.
    """
    LLAMA_SERVER_BIN = os.environ.get(
        "CPU_LLAMA_SERVER",
        "/home/bcloud/.cache/lemonade/bin/llamacpp/vulkan/llama-server"
    ...
    # Use the same Bonsai model as GPU for fast startup; >8B models will be slow on CPU anyway
    DEFAULT_MODEL = os.environ.get(
        "CPU_MODEL",
        "/home/bcloud/models/bonsai-1.7b/Bonsai-1.7B-F16.gguf"
    )

    def __init__(self, port: int = 13304):
        self.port = port
        self.process: Optional[subprocess.Popen] = None

    def start(self):
        if not os.path.exists(self.LLAMA_SERVER_BIN):
            print(f"  \u26a0\ufe0f  CPU llama-server not found at {self.LLAMA_SERVER_BIN}")
            return
        if not os.path.exists(self.DEFAULT_MODEL):
            print(f"  \u26a0\ufe0f  CPU model not found at {self.DEFAULT_MODEL}")
            return
        print(f"  Starting CPU backend (llama-server) on port {self.port}...")
        env = os.environ.copy()
        env["LD_LIBRARY_PATH"] = self.LLAMA_LD_PATH + ":" + env.get("LD_LIBRARY_PATH", "")
        self.process = subprocess.Popen(
            [self.LLAMA_SERVER_BIN,
             ...
             "--host", "127.0.0.1",
             "-ngl", "0",           # CPU only
             "-c", "2048",          # smaller context for CPU speed
            ],
            stdout=subprocess.DEVNULL,
            stderr=subprocess.DEVNULL,
            env=env,
        ...
        for _ in range(30):
            ...
            time.sleep(1)
        if self.process and self.process.poll() is None:
            print(f"  CPU backend running (pid={self.process.pid})")
        else:
            print(f"  \u26a0\ufe0f  CPU backend failed to start")
            self.process = None
    ...
    def do_GET(self):
        if self.path == "/v1/health":
            self._json(200, {
                ...
                "devices": {
                    "npu": {"backend": "FLM (NPU)", "port": backends["npu"].port,
                            "available": backends["npu"].process is not None},
                    "gpu": {"backend": "llama-server Vulkan (Radeon 8060S)",
                            "port": backends["gpu"].port,
                            "available": backends["gpu"].process is not None},
                    "cpu": {"backend": "llama-server CPU",
                            "port": backends["cpu"].port,
                            "available": backends["cpu"].process is not None},
                },
                "policy": {
                    "< 2B params": "npu",
                    "2B-8B params": "gpu",
        ...
        if self.path == "/v1/chat/completions":
            ...
            try:
                if device == "npu":
                    ...
                    resp = backends["cpu"].chat(model, messages, **extra_kwargs)
