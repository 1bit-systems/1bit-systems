...
int main() {
    ...
    // M=1 pad to M=128: A[0]=3.0, others=0
    std::vector<float> Af(MP * XK, 0.0f);
    for (int k = 0; k < XK; k++) Af[0 * XK + k] = 3.0f;
    ...
    float expected = 3.0 * 5.0 * 1024;
    int err = 0;
    for (int n = 0; n < 1024; n++) {
        float v = bf16f(Cb[0 * XN + n]);  // row 0 (M=1)
        if (fabsf(v - expected) > 2.0f) err++;
    }
    printf("M=1 pad: expected=%.0f err=%d first=%.0f\n", expected, err, bf16f(Cb[0]));
    // Check row 1 (should be 0 since A row 1 = 0)
    int err2 = 0;
    for (int n = 0; n < 1024; n++) {
        float v = bf16f(Cb[1 * XN + n]);
        if (fabsf(v) > 0.001f) err2++;
    }
    printf("Row 1 (should be 0): err=%d first=%.3f\n", err2, bf16f(Cb[1*XN]));
    return err + err2;
}
