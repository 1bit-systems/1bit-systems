/**
 * NPU Engine v3 — 2048×2048×2048 custom xclbin. 8× faster than v2.
...
int main(){
    ...
    for(int step=0;step<8;step++){
        ...
        for(int n=0;n<V;n++){double s=0;for(int k=0;k<H;k++)if(std::isfinite(lm_f[n*H+k]))s+=no[k]*lm_f[n*H+k];logits[n]=(float)s;}
	    float temp=1.0f; double sum=0; float mx=logits[0]; for(int i=1;i<V;i++)if(logits[i]>mx)mx=logits[i]; for(int i=0;i<V;i++){logits[i]=expf((logits[i]-mx)/temp);sum+=logits[i];} float r=(float)rand()/RAND_MAX*sum,acc=0; int token=0; for(int i=0;i<V;i++){acc+=logits[i];if(acc>=r){token=i;break;}}
        double ms=std::chrono::duration<double,std::milli>(std::chrono::steady_clock::now()-ts).count();
        printf("  [%d] %d (%.0f ms)\n",step,token,ms);fflush(stdout);
