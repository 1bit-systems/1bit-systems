...
int main(int argc, char** argv) {
    ...
    // ─── Async HIP stream for pipelined decode ───
    // All kernel launches go to this stream instead of nullptr (default stream).
    // hipStreamSynchronize() replaces hipDeviceSynchronize() so other streams
    // (e.g., async host->device copies) can make progress concurrently.
    hipStream_t decode_stream;
    HIP_OK(hipStreamCreate(&decode_stream));
    void* ds = static_cast<void*>(decode_stream);
    ...
    const bool is_wmma_i8     = (m.weight_format == RCPP_WEIGHT_FORMAT_WMMA_I8);
    const bool is_hadamard    = ((m.flags & H1B_FLAG_HADAMARD_ROTATED) != 0);
    if (is_hadamard) {
        fprintf(stderr, "[bitnet_decode] Hadamard rotation enabled (flag 0x%x)\n", m.flags);
    }
    ...
    // because WMMA computes a full 16×16 tile and wastes 15/16 columns.
    // WMMA is used in the prefill path (rcpp_standalone_gemm) where all outputs
    ...
    float    *logits;
    _Float16 *logits_fp16;  // FP16 logits for TQ2 LM head output
    int      *next_tok_dev;
    ...
    // In the WMMA_I8 path it uses rcpp_wmma_i8_gemv with Hadamard-rotated INT8 weights.
    // ternary_gemv dispatch — stream_ must be non-null (the decode_stream).
    auto ternary_gemv = [&](const void* packed, const int8_t* x_i8_in, float x_scale,
                            const float* row_scales, const void* normed_fp16,
                            void* out_fp16, int N, int K, hipStream_t stream_) -> rcpp_status_t {
        if (is_bonsai) {
            return bonsai_gemv_dispatch(m.weight_format, packed, normed_fp16,
                                        out_fp16, N, K, stream_);
        }
        if (is_wmma_i8) {
            return rcpp_wmma_i8_gemv(packed, x_i8_in, x_scale, row_scales,
                                     out_fp16, N, K, static_cast<void*>(stream_));
        }
        if (is_sherry_fp16) {
    ...
    auto forward_token = [&](int token_id, int pos) -> int {
        ...
        for (int l = 0; l < L; ++l) {
            ...
            if (is_bonsai) {
                // Bonsai TQ2 kernel reads FP16 activations directly — no int8 quant needed.
                // Q/K/V projections via fp16-in/fp16-out ternary GEMV.
                RC_OK(ternary_gemv(ly.q_packed_dev, /*x_i8=*/nullptr, /*x_scale=*/0.0f,
                                   /*row_scales=*/nullptr, normed, q_fp16, nh*hd,  hs_k, decode_stream));
                RC_OK(ternary_gemv(ly.k_packed_dev, nullptr, 0.0f,
                                   nullptr, normed, k_fp16, nkv*hd, hs_k, decode_stream));
                RC_OK(ternary_gemv(ly.v_packed_dev, nullptr, 0.0f,
                                   nullptr, normed, v_fp16, nkv*hd, hs_k, decode_stream));
            } else if (is_wmma_i8) {
                // WMMA_I8: Hadamard-rotate, then INT8 quant, then WMMA_i32_iu8 GEMV.
                RC_OK(rcpp_hadamard_rotate_fp16(normed, normed, hs, ds));
                ...
                RC_OK(ternary_gemv(ly.q_packed_dev, x_i8, x_scale, ly.q_scales_dev, normed, q_fp16, nh*hd,  hs_k, decode_stream));
                RC_OK(ternary_gemv(ly.k_packed_dev, x_i8, x_scale, ly.k_scales_dev, normed, k_fp16, nkv*hd, hs_k, decode_stream));
                RC_OK(ternary_gemv(ly.v_packed_dev, x_i8, x_scale, ly.v_scales_dev, normed, v_fp16, nkv*hd, hs_k, decode_stream));
            }
            ...
            // RoPE on Q and K
            RC_OK(rcpp_rope_fp16(q_fp16, pos, m.rope_theta, nh,  hd, ds));
            RC_OK(rcpp_rope_fp16(k_fp16, pos, m.rope_theta, nkv, hd, ds));
            ...
            if (is_qwen3) {
                ...
                RC_OK(rcpp_rmsnorm_fp16(o_fp16, ly.attn_sub_norm_dev, normed,
                                        m.rms_norm_eps, hs, ds));
                if (is_wmma_i8) RC_OK(rcpp_hadamard_rotate_fp16(normed, normed, hs, ds));
                RC_OK(rcpp_quantize_fp16_to_i8(normed, x_i8, x_scale_dev, hs, ds));
                float x_scale = 0.0f;
                if (!is_sherry_fp16) {
                    HIP_OK(hipMemcpy(&x_scale, x_scale_dev, 4, hipMemcpyDeviceToHost));
                ...
                RC_OK(ternary_gemv(is_wmma_i8 ? ly.o_i8_dev : ly.o_packed_dev,
                                   x_i8, x_scale, is_wmma_i8 ? ly.o_i8_scales_dev : ly.o_scales_dev,
                                   normed, o_fp16, hs, align_k(nh*hd), decode_stream));
                RC_OK(rcpp_residual_add_fp32_from_fp16(x_fp32, o_fp16, hs, ds));
            }
            ...
            if (is_bonsai) {
                RC_OK(ternary_gemv(ly.gate_packed_dev, nullptr, 0.0f,
                                   nullptr, normed, gate_fp16, is, hs_k, decode_stream));
                RC_OK(ternary_gemv(ly.up_packed_dev,   nullptr, 0.0f,
                                   nullptr, normed, up_fp16,   is, hs_k, decode_stream));
            } else {
                if (is_wmma_i8) RC_OK(rcpp_hadamard_rotate_fp16(normed, normed, hs, ds));
                RC_OK(rcpp_quantize_fp16_to_i8(normed, x_i8, x_scale_dev, hs, ds));
                float x_scale = 0.0f;
                ...
                RC_OK(ternary_gemv(is_wmma_i8 ? ly.gate_i8_dev : ly.gate_packed_dev,
                                   x_i8, x_scale, is_wmma_i8 ? ly.gate_i8_scales_dev : ly.gate_scales_dev,
                                   normed, gate_fp16, is, hs_k, decode_stream));
                RC_OK(ternary_gemv(is_wmma_i8 ? ly.up_i8_dev : ly.up_packed_dev,
                                   x_i8, x_scale, is_wmma_i8 ? ly.up_i8_scales_dev : ly.up_scales_dev,
            ...
            if (is_qwen3) {
                ...
                RC_OK(rcpp_relu2_glu_rmsnorm_fp16(gate_fp16, up_fp16, ly.ffn_sub_norm_dev,
                                                  silu_out, m.rms_norm_eps, is, ds));
            }

            if (is_bonsai) {
                // Bonsai: FP16 → ternary GEMV directly (no quant, no memcpy).
                RC_OK(ternary_gemv(ly.down_packed_dev, nullptr, 0.0f,
                                   nullptr, silu_out, down_fp16, hs, is_k, decode_stream));
            } else {
                if (is_wmma_i8) RC_OK(rcpp_hadamard_rotate_fp16(silu_out, silu_out, is, ds));
                RC_OK(rcpp_quantize_fp16_to_i8(silu_out, silu_i8, silu_scale_dev, is, ds));
                float silu_scale = 0.0f;
                if (!is_sherry_fp16 && !is_bonsai) {
                    HIP_OK(hipMemcpy(&silu_scale, silu_scale_dev, 4, hipMemcpyDeviceToHost));
                }
                RC_OK(ternary_gemv(is_wmma_i8 ? ly.down_i8_dev : ly.down_packed_dev,
                                   silu_i8, silu_scale,
                                   is_wmma_i8 ? ly.down_i8_scales_dev : ly.down_scales_dev,
                                   silu_out, down_fp16, hs, is_k, decode_stream));
            }
            RC_OK(rcpp_residual_add_fp32_from_fp16(x_fp32, down_fp16, hs, ds));
        }

        // Final norm reads FP32 residual, emits FP16.
        ...
        if (is_bonsai && m.embedding_packed_dev) {
            ...
            bonsai_tq2_gemv_launch(
                ...
                reinterpret_cast<uint16_t*>(logits_fp16),
                V, hs, nullptr);
        ...
        int next_tok;
        if (temperature <= 0.0f) {
            RC_OK(rcpp_argmax_fp32(logits, next_tok_dev, V, ds));
    ...
    auto run_turn = [&](const std::vector<int>& new_tokens, int max_new,
        ...
        for (size_t i = 0; i < new_tokens.size(); ++i) {
            ...
            HIP_OK(hipStreamSynchronize(decode_stream));
