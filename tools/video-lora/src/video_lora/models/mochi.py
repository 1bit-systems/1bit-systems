"""Mochi pipeline — Genmo's open-source T2V, strong prompt adherence."""
...
class MochiVideo(VideoPipeline):
    """Mochi text-to-video — 10B model with strong prompt adherence."""
    ...
    def __init__(
        ...
        self.pipe = MochiPipeline.from_pretrained(  # type: ignore[no-untyped-call]
            model_id,
    ...
    def generate(
        ...
        if output is None:
            output = Path(f"mochi_output_{abs(hash(prompt))}.mp4")
