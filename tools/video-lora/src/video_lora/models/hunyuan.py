"""HunyuanVideo pipeline — Tencent's flagship T2V, strong quality."""
...
class HunyuanVideo(VideoPipeline):
    """HunyuanVideo text-to-video with LoRA support."""
    ...
    def __init__(
        ...
        self.pipe = HunyuanVideoPipeline.from_pretrained(  # type: ignore[no-untyped-call]
            model_id,
    ...
    def generate(
        ...
        if output is None:
            output = Path(f"hunyuan_output_{abs(hash(prompt))}.mp4")
