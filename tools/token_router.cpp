...
static std::string run_tokenizer(const std::string& cmd, const std::string& input) {
    std::string script_file = "/tmp/tokenizer_helper.py";
    // Cache the script to a file
    static bool script_written = false;
    if (!script_written) {
        FILE* f = fopen(script_file.c_str(), "w");
        if (f) {
            fputs(tokenizer_script.c_str(), f);
            fclose(f);
            script_written = true;
        }
    }
    ...
    if (pid == 0) {
        ...
        execlp("python3", "python3", script_file.c_str(), cmd.c_str(), nullptr);
        _exit(1);
    }
    close(to_child[0]); close(from_child[1]);
    write(to_child[1], input.data(), input.size());
    close(to_child[1]);

    std::string result;
    char buf[65536];
    int n;
    while ((n = (int)read(from_child[0], buf, sizeof(buf)-1)) > 0) {
        buf[n] = 0;
        result += buf;
    }
    close(from_child[0]);
...
static std::vector<int> tokenize(const std::string& text) {
    std::string json = run_tokenizer("encode", text);
    if (json.empty() || json[0] != '[') return {};
    std::vector<int> ids;
    // Parse [1,2,3,...]
    size_t pos = 1;
    while (pos < json.size()) {
        while (pos < json.size() && (json[pos] == ' ' || json[pos] == ',' || json[pos] == '[' || json[pos] == ']')) pos++;
...
static std::string detokenize(const std::vector<int>& ids) {
    std::string json = run_tokenizer("decode",
        "[" + [&](){ std::string s; for (int id : ids) { if (!s.empty()) s += ","; s += std::to_string(id); } return s; }() + "]");
    if (json.size() >= 2 && json[0] == '"' && json.back() == '"') {
        // Simple unescape
        json = json.substr(1, json.size() - 2);
        // Handle escaped quotes
        std::string out;
        for (size_t i = 0; i < json.size(); i++) {
            if (json[i] == '\\' && i+1 < json.size()) { out += json[i+1]; i++; }
            else out += json[i];
        }
        return out;
    }
...
static void handle_request(int cl, const std::string& req, const std::string& body) {
    ...
    if (req.find("GET /v1/models") != std::string::npos) {
        std::string resp = "{\"object\":\"list\",\"data\":["
            "{\"id\":\"npu-draft\",\"object\":\"model\",\"owned_by\":\"npu\",\"description\":\"Qwen3-0.6B NPU draft (97 tok/s)\"},"
